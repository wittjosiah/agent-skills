...
- `model`: `fable`
- Instruct it to explain only and modify nothing.
...
After the explanation is complete, spawn one architectural critic each on `opus`, `fable`, and `sonnet`, all in a single message.

For each critic:
- `subagent_type`: `general-purpose`
- `model`: one of `opus`, `fable`, `sonnet`
- Instruct it to critique only and modify nothing.
