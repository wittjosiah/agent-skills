...
skill (`diagnosing-bugs`, or `diagnosing-ui` for UI bugs) or workflow; this skill is only how to get
...
2. Reproduce (yourself via browser tools whenever possible — see `diagnosing-ui`).
...
- `diagnosing-bugs` — the governing diagnosis loop (Phase 4 is where this skill is used).
- `diagnosing-ui` — the UI branch (isolation ladder, verification contract, interaction budget).
