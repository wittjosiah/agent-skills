...
Before spawning investigators, enumerate the available MCP servers. Their tools appear as `mcp__<server>__*` in the tool list; when they are deferred, `ToolSearch` with a category keyword surfaces them and reports the servers backing each match. Check `.mcp.json` in the project and `enabledMcpjsonServers` in `.claude/settings*.json` for servers configured but not yet loaded.
...
- `model`: `sonnet`
- Instruct each investigator to write nothing. Subagents reach MCP tools through `ToolSearch` regardless of posture, so no mode juggling is needed; tell them to batch every MCP tool they expect into one `ToolSearch` call.
...
- `subagent_type`: `general-purpose`
- `model`: `fable`
- Instruct it to write nothing. Its quality check spot-verifies citations, so it needs the same MCP tools the investigators used; it loads them through `ToolSearch`.
